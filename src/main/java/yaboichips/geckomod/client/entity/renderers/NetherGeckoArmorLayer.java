...
public class NetherGeckoArmorLayer<T extends NetherGeckoEntity> extends LayerRenderer<T, NetherGeckoModel<T>> {
    private final NetherGeckoModel<NetherGeckoEntity> geckoModel = new NetherGeckoModel<>();

    public NetherGeckoArmorLayer(IEntityRenderer<T, NetherGeckoModel<T>> model) {
        super(model);
    }

    @Override
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, NetherGeckoEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
