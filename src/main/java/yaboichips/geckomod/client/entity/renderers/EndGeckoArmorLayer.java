...
public class EndGeckoArmorLayer<T extends EndGeckoEntity> extends LayerRenderer<T, GeckoModel<T>> {
    private final GeckoModel<EndGeckoEntity> geckoModel = new GeckoModel<>();

    public EndGeckoArmorLayer(IEntityRenderer<T, GeckoModel<T>> model) {
        super(model);
    }

    @Override
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, EndGeckoEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
